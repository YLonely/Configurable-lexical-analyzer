...
*/
state* state_ini(int c, state *out1, state *out2, int id)
{
	state *s;
	g_state_num++;
...
/*
 ...
 Return start state
*/
state* post2nfa(char* postfix, int id)
{

	char *p = NULL;
	frag nfa_stack[100], *stack_p = NULL, e1, e2, e;
	state *s = NULL;
	state *match_state = (state*)malloc(sizeof(state));
	match_state->c = MATCH;
	match_state->out1 = match_state->out2 = 0;
	match_state->id = id;
...
#define pop() *(--stack_p)
	...
	for (p = postfix; *p; p++)
	{
		...
		case '|':
			e2 = pop();
			e1 = pop();
			s = state_ini(SPLIT, e1.start, e2.start, id);
			*e1.out = *e2.out;
			push(frag_ini(s, append(e1.out, e2.out)));
			break;
		case '?':
			e = pop();
			s = state_ini(SPLIT, e.start, NULL, id);
			*(e.out) = *(s->out2);
...
int match(char ch)
{
	...
	if (!next_list.states_num)
	{
		for (int i = 0; i < current_list.states_num; i++)
		{
			if (current_list.s[i]->c == MATCH)
			{
				return current_list.s[i]->id;
			} 
		}
		return -1;
	}
